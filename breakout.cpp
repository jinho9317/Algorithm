...
bool checkCollision(SDL_Rect rect, SDL_Point center, int rad){
    ...
    if(rect.y + rect.h < cirRect.y) return false;
    else if(rect.y > cirRect.y + cirRect.h) return false;
    else if(rect.x + rect.w < cirRect.x) return false;
    else if(rect.x > cirRect.x + cirRect.w) return false;
...
int main(int argc, char** argv){
    ...
    float rad = 9.0f;
    bool isRunning = true;
    float blockWidth = SCREEN_WIDTH / COLUMN, blockHeight = 26;
    int speed = 5, score = 0;
    int vX = 1, vY = -1;
    char scoreText[16];
    ...
    TTF_Init();
    TTF_Font* font = TTF_OpenFont("/usr/share/fonts/truetype/freefont/FreeSansBold.ttf", 24);
